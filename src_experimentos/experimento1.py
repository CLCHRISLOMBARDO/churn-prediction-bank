...
from src.lgbm_train_test import entrenamiento_lgbm
...
def lanzar_experimento_1(fecha:str ,semillas:list[int],proceso_ppal:str ="experimento"): #semillas, si queremos hacer una prediccion final, poner solo una semilla en una lista
    ...
    logger.info(f"Comienzo del experimento 1 : {name} con {len(semillas)} semillas")
    ...
    if proceso_ppal =="experimento" :
        ...
        # output_path_faetures =
        # output
        logger.info(f"LANZAMIENTO PARA EXPERIMENTO CON {len(semillas)} SEMILLAS")
        
    elif proceso_ppal == "prediccion_final":
        logger.info(f"LANZAMIENTO PARA PREDICCION FINAL CON {len(semillas)} SEMILLAS")
...
## 5. Primer Entrenamiento lgbm con la mejor iteracion y los mejores hiperparametros en [01,02,03] y evaluamos en 04 
    ...
    for semilla in semillas:
        ...
        name_1rst_train_01=f"{name}_{semilla}_1rst_train_01_SEMILLA"
        ...
        name_1rst_train_02=f"{name}_1rst_train_02_SEMILLA_{semilla}"
        ...
        name_1rst_train_03=f"{name}_1rst_train_03_SEMILLA_{semilla}"
        ...
        grafico_feature_importance(model_lgbm_01,X_train_01,name_1rst_train_01,output_path_feat_eng)
        grafico_feature_importance(model_lgbm_02,X_train_02,name_1rst_train_02,output_path_feat_eng)
        grafico_feature_importance(model_lgbm_03,X_train_03,name_1rst_train_03,output_path_feat_eng)
